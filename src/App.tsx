...
import mapsConfigJson from "./config/mapsConfig.json";
...
import { MapData, TowerTypeConfig, GameState, TargetingMode } from "./types/game";

const mapsConfig = mapsConfigJson as MapData[];
const towerConfig = towerConfigJson as Record<string, TowerTypeConfig>;

const App = () => {
  const [selectedMapId, setSelectedMapId] = useState<string>(mapsConfig[0].id);
  ...
  const handleEnemyEscape = useCallback((damage: number) => {
    const parsedDamage = Number(damage) || 0;
    setHealth((prev) => Math.max(0, prev - parsedDamage));
  }, [setHealth]);

  const handleEnemyKilled = useCallback((reward: number = 0) => {
    if (reward <= 0) return;
  ...
  const handleDifficultyChange = (newDifficulty: string) => {
    setDifficulty(newDifficulty as 'Easy' | 'Normal' | 'Hard');
    handleResetGame();
  };
  ...
  const handleCastNuke = () => {
      if (isGameOver) return;
        const ECONOMY: any = ECONOMY_BALANCE;
        if (spellCooldown > 0 || money < ECONOMY.nukeCost) return;
        applyMoneyDelta(-ECONOMY.nukeCost, 'nuke_cast', { waveNumber: wave });
        setSpellCooldown(ECONOMY.nukeCooldownSeconds);
      castNuke();
  };

  const currentFarmIncome = useMemo(() => 
    calculateFarmIncome(towers, towerConfig, ECONOMY_BALANCE).total, 
  [towers]);
